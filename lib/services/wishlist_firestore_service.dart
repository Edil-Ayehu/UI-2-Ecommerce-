...
class WishlistFirestoreService {
  ...
  static Future<bool> addToWishlist({
    ...
    try {
      ...
      await _firestore.collection(_wishlistCollection).add(wishlistItem.toFirestore());
      return true;
    } catch (e) {
  ...
  static Future<WishlistItem?> getWishlistItem(String userId, String productId) async {
    try {
      final querySnapshot = await _firestore
          .collection(_wishlistCollection)
          .where('userId', isEqualTo: userId)
